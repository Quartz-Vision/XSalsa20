...
*/
int xsalsa20_setup(xsalsa20_state *st, const unsigned char *key, unsigned long keylen,
...
/**
   ...
   @return XSALSA_OK if successful
*/
int xsalsa20_crypt(xsalsa20_state *st, const unsigned char *in, unsigned long inlen, unsigned char *out)
{
...
/**
   ...
   @return XSALSA_OK if successful
*/
int xsalsa20_keystream(xsalsa20_state *st, unsigned char *out, unsigned long outlen)
{
   unsigned char buf[64];
   unsigned long i, j;
...
{
   if (st != NULL) {
      zeromem(st, sizeof(xsalsa20_state));
   }
}
                    ...
                    unsigned char *dataout)
{
   ...
   if ((err = xsalsa20_setup(&st, key, keylen, nonce, noncelen, (int)rounds)) != XSALSA_OK) {
      return err;
   }
   if ((err = xsalsa20_crypt(&st, datain, datalen, dataout)) != XSALSA_OK) {
      xsalsa20_done(&st);
      return err;
   }
   xsalsa20_done(&st);
